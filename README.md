# Hi, I'm Shoaib Shah 👋

**Business Developer · AI-First Agency Founder · Open Source Builder**

Based in Lahore, Pakistan. I build open source tools, run cold outreach infrastructure, and lead [Riffonix Technologies](https://github.com/Riffonix) — an AI-first agency focused on automation, growth engineering, and AI-powered workflows.

---

### What I work with

- **Cold Email & Outreach:** Instantly, Smartlead, Saleshandy · DNS/SPF/DKIM/DMARC setup · Campaign management
- **AI & Automation:** Workflow automation · AI-powered tools · Lead generation systems
- **Frontend:** Vanilla JS · HTML/CSS · Browser extensions (Chrome)
- **Data:** Python · PostgreSQL · CSV processing and data pipelines
- **Platforms:** Upwork (Top Rated) · LinkedIn · GitHub

---

### Open source projects

A selection of tools I've built and shipped publicly:

| Project | What it does |
|---|---|
| [Claude Usage Tracker](https://github.com/sshnox/Claude-Usage-Tracker) | Chrome extension that shows your Claude.ai usage limits inline — no API keys, no config |
| [GitHub Stats](https://github.com/sshnox/github-stats) | Enter any username and see repo stats: stars, forks, top languages, commit activity |
| [Flashcard Quiz App](https://github.com/sshnox/flashcard-quiz-app) | Custom flashcard decks with flip animations and spaced repetition logic |
| [Weather Dashboard](https://github.com/sshnox/weather-dashboard) | Current weather + 5-day forecast via OpenWeatherMap, with recent search history |
| [Pomodoro Timer](https://github.com/sshnox/Pomodoro-Timer) | 25/5 focus timer with session counter, sound alerts, and progress ring |
| [Markdown Notes App](https://github.com/sshnox/Markdown-Notes-App) | Browser-based notes with live side-by-side markdown rendering |

---

### Currently

- Building **[Riffonix Technologies](https://github.com/Riffonix)** — AI-first agency (incorporated April 2026)
- Running cold outreach infrastructure for clients via Instantly/Smartlead
- Completing a **30-day open source build challenge** — one tool shipped per day
- Re-enrolled in **BS Data Science** at Virtual University of Pakistan

---

### Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-sshnox-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/sshnox)
[![GitHub](https://img.shields.io/badge/GitHub-sshnox-181717?style=flat&logo=github&logoColor=white)](https://github.com/sshnox)

---

*Always open to interesting projects. Reach out on [LinkedIn](https://linkedin.com/in/sshnox).*

<!--
**sshnox/sshnox** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
